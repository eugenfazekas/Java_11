...
import com.voiceRecognition.audio6.audioGramSaver.FileReaderUtil;
...
public class VoiceRecognitionDB {
	...
	public static void buildAudioDB() {
		...
		if(!VoiceRecognitionAppSetup.voiceRecognition || instanceOf || 
			!FileCheckUtil.pathExist(VoiceRecognitionAppSetup.BASE_AUDIO_PATH+"spektrum/")) 
				return;
		
		instanceOf = true;
			
		String[] voicesNamesList = 
				FileCheckUtil.getFilesOrFoldersList(VoiceRecognitionAppSetup.BASE_AUDIO_PATH+"spektrum/");
		...
		for(int i = 0; i < voicesNamesList.length; i++) {
			...
			readedPointsFile = FileReaderUtil.buildStringLinesFromInputStream(
					FileReaderUtil.buildFileStreamFromFile(
							VoiceRecognitionAppSetup.BASE_AUDIO_PATH+"spektrum/"+voicesNamesList[i]
