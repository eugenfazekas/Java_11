...
import com.voiceRecognition.audio2.recorder.AudioListener;
...
//	private static int totalValidCounter;
    ...
    public StreamRefinaryAmplitudeMethods() {
		...
		sleepTime = VoiceRecognitionAppSetup.RECORDER_MILISEC_BUFFER_LENGTH;
		...
		BUFFER_MILISEC_BEFORE_LENGTH = StreamRefinaryUtil.getBuffersLengthByMilisec(
				...
				,VoiceRecognitionAppSetup.AUDIO_BUFFER_MILISEC_BEFORE_LENGTH);
		...
		validCounterLimit = (int) ((StreamRefinaryUtil.getBuffersLengthByMilisec(
				(int)AudioListener.format.getSampleRate()
				,VoiceRecognitionAppSetup.RECORDER_MILISEC_BUFFER_LENGTH)) * 1);
		
		validLimit = validCounterLimit/10;
		validVoiceDetection = false;
		
		lastByteBuffers = new byte [VoiceRecognitionAppSetup.RECORD_BUFFER_LENGTH * 2][];
		lastByteBuffersCounter = 0;
		lastIntBuffers = new int[VoiceRecognitionAppSetup.RECORD_BUFFER_LENGTH][];
	...
	static void initBaseBufferVariables(byte[] inputBuffer) {
		...
		if(null_Counter > inputBuffer.length/8) {
		    ...
		    VoiceRecognitionMainStart.setStopAllThreads("StreamRefinaryAmplitudeMethods");
		}
		
		if(lastIntBuffersCounter == VoiceRecognitionAppSetup.RECORD_BUFFER_LENGTH) {
		...
		RECORDED_MILISEC += VoiceRecognitionAppSetup.RECORDER_MILISEC_BUFFER_LENGTH;
		loopStart = 0;
	...
	static void optimizeAmplitudeLimit() {
		...
		if(sequences == 0 ) { 
			...
			endAmplitudeLimit = (int) (((amplitude1+amplitude2+amplitude3)/3) 
					* VoiceRecognitionAppSetup.END_SOUND_THRESHOLD_MULTIPLIER);
			
			startAmplitudeLimit = (int) (((amplitude1+amplitude2+amplitude3)/3)
					*  VoiceRecognitionAppSetup.START_SOUND_THRESHOLD_MULTIPLIER);	
			
			VoiceRecognitionAppSetup.IDLE_AMPLITUDE_VOLUME = ((amplitude1+amplitude2+amplitude3)/3);
		}
	}
	 ...
	 static void saveAmplitudeStreamToExamine() {
        ...
        if (build == true && sequences > 0  &&  addedStream == false) {
        	...
        	if(!recordEndSet) {
        		...
        		BUFFER_MILISEC_AFTER_LENGTH_END = VoiceRecognitionAppSetup.AUDIO_BUFFER_MILISEC_AFTER_LENGTH 
        			+ RECORDED_MILISEC;
	 ...
	 static void info(long cycleLength) {
		...
		if( VoiceRecognitionAppSetup.amplitudeRefinary)
			Debug.debug(debud_level_DEBUG,"StreamRefinaryAmplitudeMethods INFO cycle: "+cycle + ", c.legth: "+cycleLength
				+ " mSec"+ ", Wave_Idle: "+VoiceRecognitionAppSetup.IDLE_AMPLITUDE_VOLUME
