...
public class AudioTrimSelector {
	...
	private static void buildSequenceChecks(int id) {
		...
		amplitudeSequenceFilter = new SequenceFilter((int) (VoiceRecognitionAppSetup.IDLE_AMPLITUDE_VOLUME * 1.2), 
			(int) (VoiceRecognitionAppSetup.IDLE_AMPLITUDE_VOLUME*0.8),0,0,0,0,VoiceRecognitionAppSetup.START_LENGTH_CHECK,
			VoiceRecognitionAppSetup.END_LENGTH_CHECK,AudioAnalysisThread.startedVoiceCheck.get(id).getAudioFormat());	
	}
	...
	public static void mainAudioTrim(int id) {
		...
		if(!AudioAnalysisThread.startedVoiceCheck.get(id).getNextStage().equals("trim")) {
			...
			if(VoiceRecognitionAppSetup.amplitudeTrim && VoiceRecognitionAppSetup.frequencyTrim)
	...
	private static void amplitudeTrim(int id) {
		...
		if(VoiceRecognitionAppSetup.preTrim && VoiceRecognitionAppSetup.amplitudeTrim && !VoiceRecognitionAppSetup.frequencyTrim ) {
			...
			if(trimedSequence == null && VoiceRecognitionAppSetup.continueWithNoTrim) {
	...
	private static void frequencyTrim(int id) {
		...
		if(VoiceRecognitionAppSetup.preTrim && VoiceRecognitionAppSetup.frequencyTrim && !VoiceRecognitionAppSetup.amplitudeTrim) {

			trim = new FrequencyTrimImpl(id, AudioAnalysisThread.startedVoiceCheck.get(id).getIntStream());
			
			AudioAnalysisThread.startedVoiceCheck.get(id).setByteStream(null);

			if(!VoiceRecognitionAppSetup.voiceRecognition && trimedSequence != null)	{		
