...
public class ThreadManagement implements MyThread{
	...
	private void checkThreadActions() {
		...
		while(!threadActions.isEmpty()) {
			debugMessage =  threadActions.peek().getActionName();
		...
		while(threadIsActive) {
			...
			suspendThread();
			 ...
			 try {		    
	            ...
	            Debug.debug(1,"DebugException in ThreadManagement! " +ex.getMessage() + ", debugMessage: "+debugMessage);  
			...
			sleepThread(VoiceRecognitionAppSetup.RECORDER_MILISEC_BUFFER_LENGTH);					
		}		
	}
	...
	private void initSwitch(ThreadAction threadAction) {
        ...
        Debug.debug(1,"ThreadManagement initSwitch " +threadAction.getActionName()); 
	...
	private void suspendThreadsByApplicationName(String applicationName) {
		...
		Debug.debug(debug_level_INFO, "ThreadManagement suspendThreadsByApplicationName threadsLength: " 
				+threads.size());
		
		for (MyThread thread : threads) {
		
			Debug.debug(debug_level_INFO, "ThreadManagement suspendThreadsByApplicationName thread: " 
					+" starting suspended! "+thread.getThreadObjectDetails().toString());
