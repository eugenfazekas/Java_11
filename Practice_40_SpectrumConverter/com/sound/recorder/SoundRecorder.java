    ...
    public void run() {
        ...
        try (final ByteArrayOutputStream out = new ByteArrayOutputStream();
          ...
          //  int bufferLengthInFrames = line.getBufferSize() / 8;
            ...
            System.out.println("frameSizeInBytes: "+frameSizeInBytes+" bufferLengthInFrames: "+bufferLengthInFrames +" bufferLengthInBytes: "+bufferLengthInBytes);
    ...
    public void buildByteOutputStream(final ByteArrayOutputStream out, final TargetDataLine line, int frameSizeInBytes, final int bufferLengthInBytes) throws IOException {
        ...
        while (thread != null) {
            ...
            for (int i = 0; i < data.length; i=i+2) {
       		int msb = data[i] & 0xFF; 
    	    int  lsb = data[i + 1] & 0xFF;
            short sample =  (short)((msb << 8) | lsb & 0xFF); 
            System.out.println(" buildByteOutputStream i: "+i+  "sample: "+sample + "  data[i] "+  data[i]+" data[i+1] "+data[i+1]);   
       
      
            }
                   */
        }
        System.out.println("Sound Record ByteArrayOutputStream size: "+ out.size());
    }
