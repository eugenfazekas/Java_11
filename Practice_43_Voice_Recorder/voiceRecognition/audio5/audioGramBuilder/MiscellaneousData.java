...
public class MiscellaneousData {
	...
	static int[][] buildVoiceRecognitionFrequencyGram;
	...
	static void buildVoiceRecognitionFrequencyGramString(int id) {
		
		stringBuilder = new StringBuilder();
		
		buildVoiceRecognitionFrequencyGram = VoiceRecognitionFrequencyGramBuilder.buildPoints(
				AudioAnalysisThread.startedVoiceCheck.get(id).getFrequencyMultiMap());
		
		stringBuilder.append(StringArrayUtil.buildIntArrayToString(buildVoiceRecognitionFrequencyGram[0]));
		
		for(int i = 1; i < buildVoiceRecognitionFrequencyGram.length; i++ ) 		
			stringBuilder.append(";"+StringArrayUtil.buildIntArrayToString(buildVoiceRecognitionFrequencyGram[i]));

		AudioAnalysisThread.startedVoiceCheck.get(id)
			.setVoiceReconitionFrequencyGramArray(stringBuilder.toString());
		
		AudioAnalysisThread.startedVoiceCheck.get(id).setVoiceReconitionCheckFrequencgramArray(
				buildVoiceRecognitionFrequencyGram);		
	}
}
