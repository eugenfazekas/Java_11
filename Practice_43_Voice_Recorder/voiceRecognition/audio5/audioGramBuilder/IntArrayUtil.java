...
public class IntArrayUtil {
	...
	private static int[][] multiTempArray; 
	...
	static int[][] rebuildMultiIntArray(int[][] inputgArray, int arraySize, String arrayName) {
		
		multiTempArray = new int[arraySize][];
		
		for(counter = 0 ; counter < arraySize; counter++) 
			multiTempArray[counter] =  inputgArray[counter];	
		
		Debug.debug(debug_level_INFO,"IntArrayUtil rebuildInttArray Array Name: "
				+arrayName+", old length: "+inputgArray.length+", new length: "
				+multiTempArray.length);
		
			return multiTempArray;
