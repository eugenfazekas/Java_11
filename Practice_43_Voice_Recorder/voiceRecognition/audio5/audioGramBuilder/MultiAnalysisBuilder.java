...
public class MultiAnalysisBuilder {
	...
	static void phase_2_FrequencyCheck() {  
		...
		AGBCVariables.frequencys = FFTFrecvencyCheckUtil.getFrecvencys(
			AGBCVariables.first.getSamples(), AGBCVariables.middle.getSamples()
			,AGBCVariables.last.getSamples(), ((int) AudioListener.format.getSampleRate()));
		
		FrequencyDetails.addToFrequencyWaveMap(AGBCVariables.frequencys[1]);	
		FrequencyDetails.addToMultiFrequencyWaveMap(AGBCVariables.frequencys);
	}

	static void phase_2_AddRawAudioData() {
		
		MiscellaneousData.rawAudioData[MiscellaneousData.rawDataCounter++]="\nFrequency: "
			+ AGBCVariables.frequencys+", Amplitude: "+AGBCVariables.amplitude
	...
	static void phase_2_AddToBuildSeuqnce() {
		...
		MiscellaneousData.buildSequenceStringArray[MiscellaneousData.sequenceDataCounter++]
			= MiscellaneousData.sequenceGetAmplitude()+ ":" + (AGBCVariables.frequencys) ;
	}
	...
	static void phase_3_FrequencyFiltering() {
		...
		AGBCVariables.frequencyMultiWaveMap = IntArrayUtil.rebuildMultiIntArray(
				AGBCVariables.frequencyMultiWaveMap
				,AGBCVariables.frequencyMultiWaveMapCounter,"frequencyMultiWaveMap");
		...
		AudioAnalysisThread.startedVoiceCheck.get(id).setFrequencyMultiMap(AGBCVariables.frequencyMultiWaveMap);
	}
	...
	static void phase_3_BuildVoiceRecognitionFrequencyGramArray() {
		
		MiscellaneousData.buildVoiceRecognitionFrequencyGramString(id);	
	}
