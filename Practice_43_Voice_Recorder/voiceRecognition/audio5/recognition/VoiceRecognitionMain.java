...
import voiceRecognition.audio5.recognition.area.VoiceRecognitionAreaCheck;
import voiceRecognition.audio5.recognition.multiDimension.VoiceRecognitionFrequencyGramCheck;
import voiceRecognition.audio5.recognition.points.VoiceRecognitionPointsCheck;
import voiceRecognition.audio5.recognition.scan.VoiceRecognitionScanCheck;
import voiceRecognition.audio5.recognition.slope.VoiceRecognitionSlopeCheck;
...
public class VoiceRecognitionMain {
	...
	private static void initVoiceRecognitionThreads(int id) {
		...
		if(AudioAnalysisThread.startedVoiceCheck.get(id).getVoiceReconitionCheckFrequencyGramArray() != null)
			new VoiceRecognitionFrequencyGramCheck(id,AudioAnalysisThread
					.startedVoiceCheck.get(id).getVoiceReconitionCheckFrequencyGramArray());
	}
	...
	private static int getVoiceRecognitionTestsLength() {
		...
		if(AppSetup.voiceFrequencyGramRecognition)
			testsLength++;
	...
	private static void buildMainResultBuilder(int id) {
		...
		if(startedVoiceCheck.get(id).sortedResults[4] != null) {
			mainResult[8] = 
					//startedVoiceCheck.get(id).sortedResults[4][0][startedVoiceCheck.get(id).sortedResults[4][0].length-1-sortParam] *
					 AppSetup.VOICE_RECOGNITION_FREQUENCYGRAM_CORRECTION_KONSTANT
					* (startedVoiceCheck.get(id).sortedResults[4][0][startedVoiceCheck.get(id).sortedResults[4][0].length-sortParam] / 
					   startedVoiceCheck.get(id).sortedResults[4][1][startedVoiceCheck.get(id).sortedResults[4][1].length-sortParam] );
						
			mainResult[9] = startedVoiceCheck.get(id).sortedResults[4][0][startedVoiceCheck.get(id).sortedResults[4][0].length-2];
		}
	...
	private static void sortResults(int id) {
		...
		if(startedVoiceCheck.get(id).getDbCheckResults()[4]!= null)	
			startedVoiceCheck.get(id).setSortResult(sort(startedVoiceCheck.get(id).getDbCheckResults()[4],sortParam),4);
	}
