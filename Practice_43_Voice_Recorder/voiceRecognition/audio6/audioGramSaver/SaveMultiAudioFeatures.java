...
public class SaveMultiAudioFeatures {
	...
	private static void saveMultiFrequencyGramm() {
		
		if(AudioAnalysisThread.startedVoiceCheck.get(id).getFrequencyMultiMap() 
				!= null && (AppSetup.frequencyGram)) {
			
			image = BuildAudioImage.buildFrquencyMultiImage(AudioAnalysisThread.startedVoiceCheck.get(id)
					.getFrequencyMultiMap());

			FileUtil.addImageFileToLibrary(image,"Mapped-FrequencyGram-"+speechName
											 ,MAIN_PATH+SUB_PATH+speechName+"/frequencyMultiGram");
		}
	} 
	...
	private static void saveVoiceRecognitionArray() {
		...
		if(AppSetup.voiceRecognitionData) {
			...
			String path5 = MAIN_PATH+SUB_PATH+speechName+"/voiceFrequencyGramRecognition/"+speechName
			...
			if(AppSetup.voiceRecognitionFrequencyGramData
					&& AudioAnalysisThread.startedVoiceCheck.get(id).getVoiceReconitionFrequencgramArray() != null) {	
					
				input5[0] = AudioAnalysisThread.startedVoiceCheck.get(id).getVoiceReconitionFrequencgramArray();
				
				if(pathCHeck5) 
					 FileUtil.addStreamToFile(input5[0], path5);					 
				else 
					 FileUtil.createTextFile(input5, path5);					
			}
		}
	} 	
